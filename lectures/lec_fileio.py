...
SRCFILE = os.path.join(cfg.DATADIR, 'qan_prc_2020.csv') # source file
...
# 如果之前把file read了，再for里面read第二遍时，什么都不会出现，因为read已经结束了
...
def safe_open(pth, mode):
    ...
    with open(pth) as file:
        ...
        counts=dict() #creat an empty dictionary
        for line in file:
            words=line.split()
            for word in words:
        ...
        for word,count in counts.items():
            if maxcount is None or count > maxcount:
                maxword=word
                maxcount=count
        return(f" The most frequent word is:{maxword}, and the number of times "
               f"appeared is:{maxcount}")

print(safe_open('iso.txt',mode='r'))

os.path.exists('iso.txt')

x = 2
# This will raise an exception if x == 2
if x == 2:
    raise Exception
