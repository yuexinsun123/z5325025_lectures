...
# (recommended)
prc.set_index('Date', inplace=True) # copy和源文件都被改变
prc.info()
...
prc.info()
#
#The parse_dates parameter is set to ['Date'], which indicates that the 'Date' column in the CSV file should
# be parsed and converted to a datetime data type.
...
#
#Exercise 2
print(prc.loc['2019-03-15',['Adj Close']])
...
#pct_change() to calculate returns
#calculates the percentage change between the current and a prior element in a Pandas DataFrame or Series object.
#It computes the percentage change as ((current - prior) / prior) * 100.
